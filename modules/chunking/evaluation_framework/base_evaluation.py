...
from modules.chunking.utils import rigorous_document_search, get_openai_embedding_function
...
class BaseEvaluation:
    ...
    def _get_chunks_and_metadata(self, splitter):
        ...
        for corpus_id in self.corpus_list:
            ...
            print("▶ Chunking...")
            current_documents = splitter.split_text(corpus) # custom chunker의 경우 chunking하는 부분 수정
            print("▶ Chunking...!")
            current_metadatas = []
            for document in tqdm(current_documents, desc="create metadatas..."):
                try:
    ...
    def _chunker_to_collection(self, chunker, embedding_function, chroma_db_path:str = None, collection_name:str = None, verbose:bool = False):
        collection = None
    ...
    def run(self, chunker, embedding_function=None, embedding_api_key:str|None=None, retrieve:int = 5, db_to_save_chunks: str = None):
        """
        ...
        """
        # 1. Load Question Data (question, references, corpus_id)
        self._load_questions_df()

        # 2. Set Embedding Function
        if embedding_function is None:
            if embedding_api_key is None:
                raise ValueError("embedding_api_key is required when embedding_function is None")
            else:
                embedding_function = get_openai_embedding_function(api_key=embedding_api_key)

        # 3-1. Load Chunks and Metadata (db_to_save_chunks is not None)
        collection = None
        ...
        # 3-2. Load Chunks and Metadata (db_to_save_chunks is None)
        if collection is None:
            collection = self._chunker_to_collection(chunker, embedding_function)

        # ---------------------------------------------------------------
        # 4. Load Question Data (question, corpus_id)
